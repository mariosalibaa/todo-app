...
(function () {
  ...
  // Mario, 2026-09-21: "adding the dictate idea everywhere on the hub". The page describes its form
  // (schema: name, fields [{key, hint}], context {label: [names]}, shape 'object'|'array') and gets
  // the fields back in onFilled(fields, transcript) — it fills its own inputs; nothing is saved here.
  // Server: POST /api/ai/fill (ai-fill.js) = Whisper + Claude Haiku.
  //   Admin.dictate({ host: element, schema: () => ({...}), onFilled(fields, transcript), note: true })
  // Renders into `host`: 🎤 Dictate · ✎ From a note (optional) · the "Heard: …" strip.
  A.dictate = function (opts) {
    ...
    async function send(payload, kind) {
      ...
      try {
        ...
        const j = await A.api('POST', '/api/ai/fill', { ...payload, schema });
