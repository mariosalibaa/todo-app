...
const handler = async (req, res) => {
  ...
  if (!url.startsWith('/api/')) {
    const STATIC_OK = new Set(['/manifest.json', '/hub-manifest.json', '/sw.js', '/admin-shared.js', '/phone-preview.js', '/dictate.js', '/hub-history.js', '/ajaltoun-plan-ui.js', '/emoji-picker.js', '/line-sheet.js']);
...
Answer with ONLY a JSON array: [{"text":"...","kind":"idea|work","taskId":"id or null","taskTitle":"matched task title or null","action":"link|link_done|create_task|log_only","project":"..."}]. Keep each text close to the user's own words.`;
  ...
  const refDataMatch = url.match(/^\/api\/workspaces\/([^\/]+)\/(departments|statuses|companies|priorities|changelog|users|clients|taskstatuses|folders|views|events)$/);
